...
| `verify` più veloce (un processo solo invece di sei `npm run` incatenati) | quando `verify` supera il minuto — la soglia dichiarata in [qualita.md](qualita.md). A D006 chiusa sta a 25 s, di cui più della metà è avvio di `npm` e di Node |
