...
| A14 (bis) 6 pulsanti, 6 azzeramenti, 0 anelli di fuoco          | R26    | 0044 | un solo `<button>`, con quattro forze e quattro scatole dichiarate                          | `tests/rules/buttons-pass-through-the-kit`                | ✅    | D038       |
| A14 (ter) 6 aree che scorrono, e una senza `min-height: 0`      | R27    | 0045 | una sola area che scorre, che possiede anche la riga senza cui non scorre                   | `tests/rules/scroll-passes-through-the-kit`               | ✅    | D038       |
| A14 (quater) un insieme di icone importato intero per due glifi | R28    | 0046 | i tracciati sono generati dai nomi dichiarati, e nel pacchetto entra solo ciò che si nomina | `tests/rules/icons`                                       | ✅    | D038       |
...
| R26 | Ciò che si preme passa dal kit                                                                  | ✅      | `tests/rules/buttons-pass-through-the-kit` — nessun `<button>` fuori da `UiButton.vue`, e nessun `disabled` in nessun `.vue` (ADR 0044)                                                |
| R27 | Ciò che scorre passa dal kit, e la barra si veste in un posto solo                              | ✅      | `tests/rules/scroll-passes-through-the-kit` — `overflow` che scorre e `scrollbar-*` fuori da `UiScroll.vue`; `hidden` e `visible` passano (ADR 0045)                                   |
| R28 | Un'icona viene da un insieme solo, e nessuna resta senza chi la disegni                         | ✅ / ⚠️ | `tests/rules/icons` — `glyphs.json` contro la derivazione (esatto); ⚠️ chi disegna un nome si legge dagli attributi scritti nel sorgente (ADR 0046)                                    |
...
| INV-21 | Un pulsante non si spegne mai: se non si può, c'è una frase               | ADR 0028 | `tests/rules/buttons-pass-through-the-kit` — nessun `disabled` in **nessun** `.vue`; stava nel kit fino a D038, e con R26 vale sull'applicazione                |
...
| INV-27 | Un guadagno dichiara il pool in cui atterra e quanto ne viene trattenuto  | ADR 0052 | 🔒 la firma di `income(pool, amount, withheld)`: un reddito senza regime non compila. **INV-26 manca ancora**: era di D042, che è `Chiusa` senza averlo scritto |
