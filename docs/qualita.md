...
| --- | ---------------------- | ------------------------------------------------------------------------------------------------- | ----------------- |
| G1  | `npm run typecheck`    | le regole imposte dai tipi: R04, R06, R07, R08, R09, R10, R11, INV-13 + nessun codice morto (C01) | ~3 s              |
| G2  | `npm run lint`         | le regole imposte da ESLint: R01, R03, R04, R05, R06, R10, INV-02, INV-03                         | ~2 s              |
| G3  | `npm run format:check` | il codice è formattato (C02)                                                                      | ~1 s              |
...
quattro veloci stanno sotto i quindici secondi e si eseguono a ogni modifica; la compilazione costa
un ordine di grandezza in più e serve prima di un rilascio, non prima di un salvataggio. Se `verify`
supera il minuto, è un problema da risolvere, non da tollerare.
