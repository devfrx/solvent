...
| G3  | `npm run format:check` | il codice è formattato (C02)                                                                      | ~4 s              |
| G4  | `npm run test`         | comportamento, round-trip, parità i18n, bersagli, regole strutturali, meta-test del lint          | ~6 s              |
| G5  | `npm run build`        | l'applicazione si compila davvero, main e renderer                                                | decine di secondi |

Misurati a D006 chiusa su Windows, con 150 test. Sono tempi **di parete**, quindi comprendono
...
quattro veloci stanno **sotto i trenta secondi** — a D006 chiusa, venticinque — e si eseguono a
ogni modifica; la compilazione costa un ordine di grandezza in più e serve prima di un rilascio,
non prima di un salvataggio. Se `verify` supera il minuto, è un problema da risolvere, non da
