...
| --- | ------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| N01 | **Il kernel è un punto singolo.** Un difetto nel Registry o nel Ledger si propaga a tutto         | prima nessun componente aveva questo potere                                                          | ~500 righe, testate per intero, pure, senza I/O                                                                  | basso                                                  |
| N02 | **`order` è una risorsa globale.** Con molti sistemi, ragionare su chi va prima diventa difficile | l'ordinamento globale è un accoppiamento nascosto fra sistemi che non si conoscono                   | costanti nominate raggruppate per fase; il numero di sistemi cresce lentamente (ADR 0014)                        | **medio**                                              |
...
| N07 | **La documentazione può disallinearsi dal codice**                                                | è il difetto classico di un set di documenti ampio: mente con autorevolezza                          | ADR append-only; deleghe che si **chiudono**; l'architettura si aggiorna nello stesso task che sposta un confine | **alto — è il rischio principale di questo approccio** |
